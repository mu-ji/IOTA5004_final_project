...
y_train = np.array(y_trajectory_half)
...
class f_t(nn.Module):
    ...
    def __init__(self):
        ...
        self.g = nn.Parameter(9.8*torch.ones(1, ), requires_grad=True)
...
# 定义参数
g = f_model.g.detach().numpy()[0]
k = f_model.k.detach().numpy()[0]
print(g)
print(k)
# 定义初始条件
y0 = y_train[0]
v_y0 = (y_train[1] - y_train[0])/0.033

print('initial state:',y0,v_y0)

# 定义时间点
ts = np.linspace(0, 0.033*12, 12) 
...
v_y_with_resistance = solution_with_resistance[:, ]

plt.plot(ts,y_with_resistance)
plt.plot(t_train, model( ts_torch )[:,0:1].detach().numpy(),lw = 2)
plt.show()